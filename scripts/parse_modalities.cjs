...
function parseFiles() {
            ...
            for (let i = 0; i < json.length; i++) {
                ...
                if (isSuper) {
                    if (superTotalIndex > -1 && typeof row[superTotalIndex] === 'number') {
                        ...
                        // Check if it's slightly shifted
                        // Just a safety check, usually it's correct.
                    }
                } else {
    ...
    fs.writeFileSync('../src/data/modalities_data.json', JSON.stringify(data, null, 2));
    console.log('Saved to src/data/modalities_data.json');
}
